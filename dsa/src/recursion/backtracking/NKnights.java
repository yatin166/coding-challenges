...
public class NKnights {
    ...
    private static boolean isSafe(boolean[][] board, int row, int col) {
        ...
        if (row + 1 < board.length && col + 2 < board.length && board[row+1][col+2]) return false;

        // check bottom 2 pos
        if (row + 2 < board.length && col + 1 < board.length && board[row+2][col+1]) return false;
        if (row + 2 < board.length && col - 1 >= 0 && board[row+2][col-1]) return false;

        // check left 2 pos
        if (row + 1 < board.length && col - 2 >= 0 && board[row+1][col-2]) return false;
